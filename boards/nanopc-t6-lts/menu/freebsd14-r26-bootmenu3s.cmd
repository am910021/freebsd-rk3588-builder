...
setenv boot_freebsd_image 'setenv stdout serial,vidconsole; setenv stderr serial,vidconsole; coninfo; mmc dev 1; run usb_vbus_on; echo Booting FreeBSD 14.3 from mmc 1:1; if load mmc 1:1 ${fdt_addr_r} /dtb/rk3588-nanopc-t6.dtb; then if fdt addr ${fdt_addr_r} && fdt resize 0x100000; then if test -n "${rk_boot_storage}"; then fdt set /chosen rockchip,boot-storage "<${rk_boot_storage}>"; else echo Unknown U-Boot storage; fi; if run apply_fdt_overlays; then if load mmc 1:1 ${kernel_addr_r} /EFI/FreeBSD/loader.efi || load mmc 1:1 ${kernel_addr_r} /EFI/BOOT/BOOTAA64.EFI; then bootefi ${kernel_addr_r} ${fdt_addr_r}; fi; fi; fi; fi; echo FreeBSD boot failed'
